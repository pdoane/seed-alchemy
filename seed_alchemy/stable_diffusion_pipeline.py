# Code from various diffusers pipelines
# https://github.com/huggingface/diffusers/blob/v0.15.1/src/diffusers/pipelines/stable_diffusion/pipeline_stable_diffusion.py
#
# Too much variation between pipeline implementations right now that need to be resolved locally but this can likely
...
class StableDiffusionPipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin):
        ...
        if controlnet is not None:
            ...
            if isinstance(controlnet, MultiControlNetModel):
                if isinstance(prompt, list):
                    logger.warning(
                        f"You have {len(controlnet.nets)} ControlNets and you have passed {len(prompt)}"
            ...
            if isinstance(controlnet, ControlNetModel):
                ...
                if len(controlnet_conditioning_image) != len(controlnet.nets):
                    raise ValueError(
            ...
            # Check `controlnet_guidance_start` and `controlnet_guidance_end`
    ...
    def __call__(
        ...
        Args:
            ...
            controlnet_conditioning_image (`torch.FloatTensor`, `PIL.Image.Image`, `List[torch.FloatTensor]`, `List[PIL.Image.Image]`,
        ...
        elif controlnet_conditioning_image is not None:
        ...
        # 4. Preprocess image and controlnet_conditioning_image
        if image is not None:
            ...
            for i, t in enumerate(timesteps):
                ...
                if controlnet is not None:
                    ...
                    if (
                        ...
                        and current_sampling_percent <= controlnet_guidance_end
                    ):
